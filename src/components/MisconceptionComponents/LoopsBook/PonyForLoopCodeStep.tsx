...
export function PonyForLoopCodeStep({setAllowNext} : {setAllowNext: React.Dispatch<React.SetStateAction<boolean>>}) {

    const windowScale = GetWindowScale()

    const [currentLine, setCurrentLine] = useState(0)
    const [currentImage, setCurrentImage] = useState("/LoopsBook/PonyRiding/for_loop_pony_riding.svg")
        ...
        <div className='flex flex-col gap-5 text-center' style={{zoom: windowScale}}>
            <div className='flex gap-5'>
                <div className='flex flex-col w-1/2 justify-center'>
                    <CodeStep props={{code: code, skipLines: [1,4], enableNext: true, getLine: getLine}} loop={{exists: true, lines: [2, 3], times: 3, getIteration: getIteration}}/>
                </div>

    ...
    return (
        <div className="flex flex-row items-start justify-between text-xs xl:text-lg">
            ...
            <div className="flex flex-col">
                <div className="p-1">
                    <div className="flex flex-col" onDrop={(e) => handleOnDropCondition(e, "condition")} onDragOver={(e) => handleDragOver(e)}>
                        <label htmlFor="ifCondition" className="">
                            ...
                            <input key={"ifCondition"} type="text" className="w-40 outline-black rounded-2xl outline-dotted text-center px-8" placeholder={props?.type ? "Type Here" : "Drag Condition Here"} disabled={!props.type && !isFireFox} defaultValue={props.condition} onChange={(e) => handleInputChangeCondition(e)} />
                        </label>
                    </div>
                    <div className="flex flex-col" onDrop={(e) => handleOnDropStatement(e, "statement")} onDragOver={(e) => handleDragOver(e)}>
                        ...
                        <input key={"ifConditionStatement1"} type="text" className="w-40 outline-black rounded-2xl outline-dotted text-center" placeholder={props?.type ? "Type Here" : "Drag appropriate action here"} disabled={!props.type && !isFireFox} defaultValue={game.statement} onChange={(e) => handleInputChangeStatement(e)} />
                    </div>
                </div>
                ...
                {wrong &&
                    <div className="bg-red-100 border border-red-400 text-red-700 p-1 rounded-xl" role="alert">
                ...
                {good &&
                    <div className="bg-green-100 border border-green-400 text-green-700 p-1 rounded-xl" role="alert">

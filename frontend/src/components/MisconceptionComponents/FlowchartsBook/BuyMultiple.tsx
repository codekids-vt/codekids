...
export function BuyMultiple({
  props,
  setAllowNext,
}: {
  props: any | IBuyMultipleProps;
  ...
  const getLine = (lineNumber: number) => {
    ...
    if (lineNumber < 3) {
      ...
      setP2Text(
        "Now the program needs to make a decision. It can either follow the True arrow or the False arrow.",
      );
      setImageDim([1000, 1000]);
    } else if (lineNumber === 7 || lineNumber === 8) {
      setCurrentImage("/FlowchartsBook/BuyMultiple/example2_2.svg");
      setP2Text(
        "The program follows the False arrow since total is greater than your_money. This is the same as the 'else' in the code.",
      );
      setImageDim([1000, 1000]);
